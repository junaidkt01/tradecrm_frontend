...
const PackagesCard = ({ data, onButtonClick }: { data: Package; onButtonClick: (method: string, bonus?: Package) => void; }) => {
    ...
    const { deletePackages } = usePackages({})

    const handleDeleteBonus = (id: any) => {
        if (confirm('Are you sure you want to close this ticket?')) {
            deletePackages.mutate(id, {
                onSuccess: () => {
        ...
        <div className="packages-card" >
            <div className="packages-card-top" >
                <div className="title" >
                    ...
                    <div style={{ position: "relative" }} >
                        <div className="card-menu-icon" >
                            <div className={`live-demo ${data.account_type === "live" ? "live" : "demo"}`}><p>{data.account_type === "live" ? "Live" : "Demo"}</p></div>
                ...
                <div className="packages-card-content" >
                    <p>Max. Leverage: <strong>{data.max_leverage}</strong></p>
                    <p>Manager Group: <strong>{data.manager_id}</strong></p>
                    <p>Plan Flow: <strong>{data.plan_flow}</strong></p>
                    <p>Created On: <strong>{data.created_at}</strong></p>
                </div>
            </div>
        </div >
    )
