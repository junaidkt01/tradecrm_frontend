...
const AddManagerForm = ({ togglePopup, data }: { togglePopup: () => void; data: ManagerType | undefined }) => {
    const handleTogglePopup = () => {
        togglePopup()
    }

    const { createManager, updateManager } = useManagers();
    ...
    const handleAccountTypeChange = (type: 'Demo' | 'Live') => {
        setNewManager((prev: any) => ({ ...prev, server_type: type }));
    };
    ...
    const handleFormSubmit = async () => {
        const mutation = data ? updateManager : createManager;

        try {
            await mutation.mutateAsync(newManager);
    ...
    return (
        <div className="add-user-for add-popup-window" >
                ...
                <div className="add-user-form-controller" >
                    ...
                    <div className="account-type" >
                        <button
                            ...
                            value={newManager?.server_type === 'Demo' ? 'Demo' : ''}
                            className={newManager?.server_type === 'Demo' ? 'active' : ''}
                        >Demo</button>
                        <button
                            onClick={() => handleAccountTypeChange('Live')}
                            value={newManager?.server_type === 'Live' ? 'Live' : ''}
                            className={newManager?.server_type === 'Live' ? 'active' : ''}
                        >Live</button>
                    </div>
                </div>
            ...
            <form className="add-user-form-box" onSubmit={handleFormSubmit} >
                <div className="add-manager-form">
                    <div>
                        <label htmlFor="">Server Name</label>
                        <input
                            onChange={handleInputChange}
                            value={newManager?.server_name}
                            name="server_name"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            name="manager_id" type="text" placeholder="Enter Manager ID" />
                    </div>
                    <div>
                        <label htmlFor="">Password</label>
                        <input
                            onChange={handleInputChange}
                            value={newManager?.server_pwd}
                            name="server_pwd" type="text" placeholder="Enter Password" />
                    </div>
                    <div>
                        <label htmlFor="">Server Config</label>
                        <input
                            onChange={handleInputChange}
                            value={newManager?.server_config}
                            name="server_config" type="text" placeholder="Enter Server Config" />
                    </div>
                </div>
