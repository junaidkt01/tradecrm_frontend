...
// import { Manager } from "../../../utils/contents/configuration/manager";
...
const ManagerCard = ({ data, onButtonClick }: { data: ManagerType; onButtonClick: (method: string, bonus?: ManagerType) => void; }) => {
    const [isMenuOpen, setIsMenuOpen] = useState(false);

    const onChangeTogglePopup = (title: string) => {
        if (title === "Delete") {
            handleDeleteBonus(data.manager_id)
        }
        if (title === "Edit") {
        ...
        <div className="manager-card" >
            <div className="manager-card-top" >
                    ...
                    <div style={{ position: "relative" }} >
                        <div className="card-menu-icon" >
                            <svg onClick={handleClick} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M13.9602 3.73916L15.3569 2.33249C15.6481 2.03943 16.043 1.87479 16.4548 1.87479C16.8665 1.87479 17.2614 2.03943 17.5526 2.33249C17.8438 2.62556 18.0073 3.02304 18.0073 3.43749C18.0073 3.85195 17.8438 4.24943 17.5526 4.54249L8.76078 13.3917C8.32307 13.832 7.7833 14.1556 7.19019 14.3333L4.9672 15L5.62954 12.7625C5.80612 12.1655 6.12765 11.6222 6.5651 11.1817L13.9602 3.73916ZM13.9602 3.73916L16.1443 5.93749M14.9024 11.6667V15.625C14.9024 16.1223 14.7061 16.5992 14.3568 16.9508C14.0074 17.3025 13.5336 17.5 13.0395 17.5H4.34625C3.85219 17.5 3.37837 17.3025 3.02901 16.9508C2.67966 16.5992 2.4834 16.1223 2.4834 15.625V6.87499C2.4834 6.37771 2.67966 5.9008 3.02901 5.54917C3.37837 5.19754 3.85219 4.99999 4.34625 4.99999H8.27892" stroke="#62636C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        </div>
                ...
                <div className="manager-card-content" >
                    <p>Server Name: <strong>{data.server_name}</strong></p>
                    <p>Server Type: <strong>{data.server_type}</strong></p>
                    <p>Manager ID: <strong>{data.manager_id}</strong></p>
                    <p>Server Config: <strong>{data.server_config}</strong></p>
                </div>
            </div>
        </div>
    )
