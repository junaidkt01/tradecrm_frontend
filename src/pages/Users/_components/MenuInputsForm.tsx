...
import KYCDocs from "./_menuForms/KYCDocs"
import AccountDetails from "./_menuForms/AccountDetails"
import { menuPopupContents } from "../../../utils/usersMenuPopup"

const MenuInputsForm = ({ toggleMenuPopup, userData }: { toggleMenuPopup: (menuType: string) => void; userData: { menuType: string; userData: { userId: string | number; name: string } | undefined } }) => {
    const [selectTab, setSelectTab] = useState("")

    const handleTogglePopup = (closeMenu: string) => {
        toggleMenuPopup(closeMenu)
    }
    ...
    return (
        <div className="menu-inputs-form" >
            <div className="add-user-for add-popup-window" >
                ...
                <div className="users-inputs-filter-btns" >
                    {
                        // menuPopupContents.map((item, i) => <FilterBtn title={item.title} onClick={() => setSelectTab(item.title)} key={i} isActive={selectTab === item.title} />)
                        menuPopupContents.map((item, i) => <FilterBtn title={item.title} onClick={() => setSelectTab(item.title || userData.menuType)} key={i} isActive={(selectTab || userData.menuType) === item.title} />)
                    }
                </div>

                <div style={{ marginTop: "20px" }} >
                    {selectTab === "Change password" && <h3>Change password</h3>}
                    {selectTab === "Upload View Document" && <KYCDocs />}
                    {selectTab === "Accounts" && <AccountDetails />}
                    {selectTab === "IB Linkage" && <h3>IB Linkage</h3>}
                    {selectTab === "Edit Workspace" && <h3>Edit Workspace</h3>}
                    {selectTab === "Bank Details" && <h3>Bank Details</h3>}
                    {selectTab === "Send Mail" && <h3>Send Mail</h3>}
                    {selectTab === "Reset Password" && <h3>Reset Password</h3>}
                    {selectTab === "Sessions" && <h3>Sessions</h3>}
                    {selectTab === "Reports" && <h3>Reports</h3>}
                </div>
            </div>
        </div >
    )
