...
import { IBcomisnOvrl, IBComisnTrnsctn, IBdlyComisn } from "../../components/IbAndSubIbTable/IbAndSubIbTable";
import { iBcomisnOvrl, iBComisnTrnsctn, iBdlyComisn } from "../../utils/IbAndSubIbTable";
import Breadcrumbs from "../../components/Buttons/Breadcrumbs/Breadcrumbs";

const Commission = () => {
    const [walletTable, setWalletTable] = useState("overall");

    return (
        <div className="commission pages-wrapper" >
            <Breadcrumbs title="IB / Commission" />
            <div className="wallet-page-top" style={{ display: "grid" }} >
                ...
                <div className="ledger-profile-wallet-transfer-btns" >
                    <button onClick={() => setWalletTable("overall")} className={walletTable === "overall" ? "active" : ""} >Overall</button>
                    <button onClick={() => setWalletTable("daily_commission")} className={walletTable === "daily_commission" ? "active" : ""} >Daily Commission</button>
                    <button onClick={() => setWalletTable("transaction")} className={walletTable === "transaction" ? "active" : ""} >Transaction</button>
                </div>
            </div>

            <div className="wallet-page-table" >
                {
                    walletTable === "overall" && <IBcomisnOvrl tableData={iBcomisnOvrl} />
                }

                {
                    walletTable === "daily_commission" &&
                    <>
                        <p>Filter by date</p>
                        <IBdlyComisn tableData={iBdlyComisn} />
                    </>
                }
                {
                    walletTable === "transaction" &&
                    <>
                        <p>helo</p>
                        <h2>hleo 2</h2>
                        <IBComisnTrnsctn tableData={iBComisnTrnsctn} />
                    </>
                }
            </div>
        </div>
